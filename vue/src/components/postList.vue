<template>
  ...
  <v-container fluid>
    <div v-if="topPost == false">
      ...
      <v-card v-for="post in postList" :key="post.id" hover style="margin-top:10px" outlined
              @click="toPostPage(post.postId)">
        <v-card-title>
          <span class="text-h5 text--primary font-weight-black text-uppercase">{{ post.title }}</span>
        </v-card-title>
        <v-card-subtitle>
          <v-row no-gutters>
            <v-col class="col-1 text-truncate">
              <span class="text-h6 font-weight-medium " style="max-width: 150px;"><v-icon>mdi-account-circle-outline</v-icon> {{ post.userName }}</span>
            </v-col>
            <v-col class="col-9 text-truncate">
              <span class="text-h6 font-weight-medium black--text "
                    style="font-style: italic">&emsp; &emsp; {{ post.content }}</span>
            </v-col>
            <v-col class="col-2 text-truncate">
              <span class="text-h6 font-weight-medium d-inline-block text-truncate"
                    style="max-width: 150px; float:right"> {{ post.gmtModified }}</span>
            </v-col>
          </v-row>
        </v-card-subtitle>
      </v-card>
    ...
    <div v-else-if="topPost == true">
      ...
      <v-card v-for="post in postList.slice(0,3)" :key="post.id" hover style="margin-top:10px" outlined
              @click="toPostPage(post.postId)">
        <v-card-title>
          <span class="text-h5 text--primary font-weight-black text-uppercase">{{ post.title }}</span>
        </v-card-title>
